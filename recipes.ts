...
recipeRouter.get("/recipe/:id", async (req, res) => {
  try {
    ...
    let result = await client.query(
      `select recipe.id, user_name, title as recipe_title, video, calories, content, cuisine.name as cuisine, image, ingredient_name, amount, unit, profile_pic from recipe join cuisine on recipe.cuisine_id = cuisine.id 
